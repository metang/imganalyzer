...
def run_distributed_worker(
    ...
    for raw in path_mapping or []:
        if "=" not in raw:
            raise typer.BadParameter(
                "Path mappings must use SOURCE_PREFIX=LOCAL_PREFIX, for example "
                "'Z:\\photos=/Volumes/photos'"
            )
        source_prefix, local_prefix = raw.split("=", 1)
        source_prefix = source_prefix.strip()
        local_prefix = local_prefix.strip()
        if not source_prefix or not local_prefix:
            raise typer.BadParameter("Path mappings require both SOURCE_PREFIX and LOCAL_PREFIX")
        path_mappings.append((source_prefix, local_prefix))
...
def profile_report(
    ...
    if phase_stats:
        phase_names = {0: "objects", 1: "caption+kw", 2: "faces+embed"}
