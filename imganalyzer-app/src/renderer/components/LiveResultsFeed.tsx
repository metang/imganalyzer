  ...
  return (
    <div className="flex-1 overflow-y-auto text-xs font-mono">
      {results.map((r) => {
        ...
        return <ResultRow key={r.id} result={r} nodeColor={nc} />
      })}
    </div>
  )
}
...
// ── ResultRow ────────────────────────────────────────────────────────────────
// Memoised so that appending a new result at the head of the feed doesn't
// re-render every existing row — the row's props (result + its node color)
// are stable across renders once produced.

interface ResultRowProps {
  result: BatchResult
  nodeColor: { text: string; dot: string }
}

const ResultRow = memo(function ResultRow({ result: r, nodeColor: nc }: ResultRowProps) {
  return (
    <div
